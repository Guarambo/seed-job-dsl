    ...
    stages {
        ...
        stage("Deploy"){
            steps{
                ...
                sh 'scp -r testEAR root@172.18.0.4:/usr/share/compiled-app/'

                /*deploy.jython = Script para ejecutar una aplicacion*/
                sh 'ssh root@172.18.0.4  /opt/IBM/WebSphere/AppServer/bin/wsadmin.sh -f /usr/share/compiled-app/deploy.jython -lang jython -conntype SOAP -host localhost -user wsadmin -password R4QrGxtf'
            }
        }
    }
}